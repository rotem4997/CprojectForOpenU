...
void print_words_to_file(FILE * file_to_write) {
    ...
    while (it){ /* for each word */
        ...
        /*create word by type */
        if(it->ocw){
            word_bits = (( (1 <<2) & 0xFF)<<32) | (it->ocw->op_code) ;
        } else if (it->dlw){
            word_bits = ( it->dlw->are<<32) | (unsigned short int)( it->dlw->data);
        }
...
data_word *new_data_word(unsigned int *are , int *value , char *init_missing_symbol, data_word t){
    ...
    if (init_missing_symbol != NULL) {
        new_data_word->symbol_need_to_be_filled = (char *) safe_malloc(sizeof(char ) * ASCII_MAX_LENGTH_VALUE);
